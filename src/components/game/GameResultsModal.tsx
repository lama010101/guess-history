...
const GameResultsModal = ({
  ...
  useEffect(() => {
    if (showResults && selectedLocation) {
      ...
      const isPerfectLoc = isPerfectLocation(selectedLocation, currentImage.location, locationThreshold);
      ...
      import('@/utils/achievementUtils').then(({
        getUserAchievements
      }) => {
  ...
  return <Dialog open={showResults} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        {/* Image preview - moved directly under description */}
        <div className="relative aspect-video rounded-md overflow-hidden bg-black/10 mb-6">
          <img src={currentImage.src} alt={currentImage.description || "Historical image"} className="w-full h-full object-contain" />
        </div>
        
        <div className="space-y-6">
          {/* Location Section */}
          <div className="border rounded-md overflow-hidden py-[20px]">
            <div className="bg-muted p-3 flex justify-between items-center px-[11px] py-0 my-[150px]">
            ...
            <div className="p-4 mx-0 py-0 my-0 px-0">
              <p className="text-sm mb-3">
                You were <span className="font-medium">{Math.round(distanceKm)} km</span> away from the actual location.
              </p>
              
              {selectedLocation && <div className="h-48 rounded-md overflow-hidden border mb-3 my-[132px]">
                  <ResultVisualization actualLocation={currentImage.location} guessedLocation={selectedLocation} isVisible={true} circleRadius={300000} showConnectionLine={true} />
                </div>}
              
              {perfectLocation && <div className="flex items-center text-green-600 gap-2 mt-3">
                  <AchievementBadge type="location" size="sm" count={achievements.perfectLocations} />
                  <span className="text-sm font-semibold">Perfect location guess!</span>
                </div>}
              
              {locationHintUsed && <div className="bg-amber-100 text-amber-800 dark:bg-amber-900/20 dark:text-amber-400 text-xs p-3 rounded mt-3">
          ...
          <div className="border rounded-md overflow-hidden">
            ...
            <div className="p-4">
              <p className="text-sm">
                ...
                {yearDifference !== 0 && <> ({yearDifference > 0 ? `${yearDifference} years earlier` : `${Math.abs(yearDifference)} years later`})</>}
              </p>
              
              {perfectYear && <div className="flex items-center text-green-600 gap-2 mt-3">
                  <AchievementBadge type="year" size="sm" count={achievements.perfectYears} />
                  <span className="text-sm font-semibold">Perfect year guess!</span>
                </div>}
              
              {yearHintUsed && <div className="bg-amber-100 text-amber-800 dark:bg-amber-900/20 dark:text-amber-400 text-xs p-3 rounded mt-3">
          ...
          {/* Combo Achievement */}
          {perfectCombo && <div className="border border-green-300 dark:border-green-800 rounded-md p-4 bg-green-50 dark:bg-green-900/20">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Trophy className="h-5 w-5 text-green-600 dark:text-green-400" />
                  <span className="font-medium text-green-800 dark:text-green-300">Perfect Combo!</span>
                </div>
                <AchievementBadge type="combo" size="sm" count={achievements.perfectCombos} animated={true} />
              </div>
          ...
          <div className="bg-primary/5 p-4 rounded-lg">
            <div className="flex justify-between items-center mb-2">
              <div className="text-sm w-full">
                ...
                {hintPenalty > 0 && <div className="flex justify-between text-amber-600 mb-2">
