...
const realClient = (g.__supabaseClient ||= createClient<Database>(
  SUPABASE_URL,
  SUPABASE_PUBLISHABLE_KEY,
  clientOptions as any
));

const shouldUseFake = (): boolean => {
  try {
    const w: any = (globalThis as any);
    if (w && w.__USE_SUPABASE_FAKE) return true;
    if (typeof localStorage !== 'undefined') {
      const v = localStorage.getItem('useSupabaseFake');
...
const getClient = () => (shouldUseFake() ? supabaseFake : realClient);
