-- Migration: Improve game prep randomness using seeded hash ordering
-- Date: 2025-10-12

-- Redefine RPC to select candidates using seeded hash order across all eligible images
...
BEGIN
  ...
  -- Build eligible set (ready and excluding player's history if provided; respect year range)
  WITH eligible AS (
    SELECT i.id
  ...
  ranked AS (
    -- Seeded, deterministic pseudorandom order across the WHOLE eligible set
    SELECT id
    FROM eligible
  ...
  -- Return deterministic in-game order using the same seed
...
-- Ensure grants are present for the updated signature
