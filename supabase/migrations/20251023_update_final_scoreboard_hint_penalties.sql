...
DROP FUNCTION IF EXISTS public.get_final_scoreboard(text);

CREATE FUNCTION public.get_final_scoreboard(
  p_room_id text
)
RETURNS TABLE(
  user_id uuid,
...
BEGIN
  ...
  SELECT
    sp.user_id,
    COALESCE(sp.display_name, pr.display_name, 'Player') AS display_name,
    ...
    COALESCE(SUM(rr.acc_debt), 0)::numeric(10,2) AS total_acc_debt
  ...
  GROUP BY sp.user_id, sp.display_name, pr.display_name
